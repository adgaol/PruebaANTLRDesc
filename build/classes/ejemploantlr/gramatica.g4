...
@members {
    Writer writer = new Writer("gramatica.g4","./descendentor","cadena.txt",true);
}
...
exp  returns [Exp expO] 
    ...
    {
        Exp expO=new Exp();
        Node node=writer.addPasoNoTerminal("EXP",  null,  null,  expO,  false,  null, null);
    } 
...
a [Node nodeAnt,Boolean haveBrother,Integer her]  returns  [A aO]
    ...
    { 
        A aO=new A();
        Node node=writer.addPasoNoTerminal("A",  "valor",  "result",  aO,  haveBrother,  her.toString(), nodeAnt);
    }
masO=mas[node, true] bO=b[node, true] aeO=a[node, false, Integer.parseInt(((AContext)_localctx).bO.bO.getValue())+her] 
    ...
    {
        A aO=new A();     
        writer.addPasoLambda("A",  "valor",  "result",  her.toString(),  "{A.result=A.valor;}",  aO,  haveBrother, nodeAnt);
        _localctx.aO=aO;
    }
    ;
b  [Node nodeAnt,Boolean haveBrother] returns  [B bO]
    ...
    {
        B bO=new B();
        Node node=writer.addPasoNoTerminal("B",  null,  "result",  bO,  haveBrother,  null, nodeAnt);
    } 
...
c [Node nodeAnt,Boolean haveBrother,Integer her]   returns  [C cO]
    ...
    {
        C cO=new C();
        Node node=writer.addPasoNoTerminal("C",  "valor",  "result",  cO,  haveBrother,  her.toString(), nodeAnt);
    }  
porO=por[node, true] numO=number[node, true] ceO=c[node, false, her*Integer.parseInt(((CContext)_localctx).numO.numberO.getValue())] 
    ...
    {
        C cO=new C();    
        writer.addPasoLambda("C",  "valor",  "result",  her.toString(),  "{C.result=C.valor;}",  cO,  haveBrother, nodeAnt);
        _localctx.cO=cO;
    }
    ;
